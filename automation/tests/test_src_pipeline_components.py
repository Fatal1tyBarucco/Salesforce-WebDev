...
def test_markdown_generator_generate(tmp_path: Path) -> None:
    ...
    content_map = {
        "apex": ["Feature 1", "Feature 2"],
        "lwc": []
    }
    
    generated_files = generator.generate(release, content_map, "https://fake-source.com")
...
def test_readme_updater_update_flow(tmp_path: Path) -> None:
    ...
    readme_file.write_text("Some Header\n<!-- RELEASE_INDEX_START -->\nold index\n<!-- RELEASE_INDEX_END -->\nSome Footer")
    ...
    with patch("src.readme_updater.KNOWN_RELEASES", [ReleaseInfo(name="Summer '26", release_id=262, slug="summer_26")]):
