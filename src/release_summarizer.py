...
class ReleaseSummary:
    ...
    executive_summary: str  # Up to 5000 chars
    ...
    category_summaries: dict[str, str]  # category_name -> summary (up to 1000 chars each)
    confidence: float
...
class ReleaseSummarizer:
    ...
    async def summarize(self, release_slug: str, lang: str = "pt_BR") -> ReleaseSummary | None:
        """Generate a comprehensive summary for a release.

        Args:
            release_slug: The release directory name (e.g., 'summer_26').
            lang: Language locale ("pt_BR" or "en_US"). Determines which
                summary cache to use. Falls back to pt_BR if the language-specific
        ...
        cache_file = (
            release_dir / ".summary_cache_en.json"
            if lang == "en_US"
            else release_dir / ".summary_cache.json"
        )
        if cache_file.exists():
            try:
                ...
                # Check 4: cache was generated by fallback (metadata-only, no LLM)
                if cache.get("fallback") is True:
                    logger.info(
                        "Cache for %s has fallback=True — forcing LLM regeneration "
                        "(cache was generated without AI).",
                        release_slug,
                    )
                    cache_is_valid = False
        ...
        system_prompt = (
            ...
            "{\n"
            '  "executive_summary": "Comprehensive executive overview of the release (2000-5000 characters). '
            "Cover the scale (number of features/categories), the primary focus areas, key innovations, "
            "what this release means for Salesforce customers, and strategic direction. "
            'Be thorough and detailed, not a brief overview.",\n'
            '  "business_impact": "A detailed paragraph explaining the business value. '
            ...
            "  ],\n"
            '  "category_summaries": {\n'
            '    "Category Name": "AI-generated summary of this category (up to 1000 characters). "\n'
            "  },\n"
            ...
            "- 'executive_summary' must be comprehensive (2000-5000 chars), not a brief overview\n"
            ...
            "- 'category_summaries' must include a summary for EACH category provided in the input. "
            "Each summary should highlight the most impactful features, trends, and business relevance. "
            "Up to 1000 characters per category.\n"
    ...
    def _parse_llm_response(
        ...
        try:
            ...
            raw_cat_summaries = data.get("category_summaries", {})
            category_summaries: dict[str, str] = {}
            for cat_name, cat_summary in raw_cat_summaries.items():
                if isinstance(cat_summary, str) and cat_summary.strip():
                    category_summaries[cat_name] = cat_summary[:1000]
    ...
    def _generate_fallback_summary(
        ...
        # Build category summaries (fallback — no LLM)
        category_summaries: dict[str, str] = {}
        for name, count in category_counts.items():
            features = category_features.get(name, [])
            feature_list = ", ".join(features[:5]) if features else ""
            summary = f"{name} conta com {count} novos recursos nesta release."
            if feature_list:
                summary += f" Destaques: {feature_list}."
            category_summaries[name] = summary[:1000]
