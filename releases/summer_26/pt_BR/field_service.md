<div style="padding:8px 12px;margin-bottom:16px;border:1px solid #d0d7de;border-radius:6px;background:#f6f8fa;font-family:system-ui,sans-serif;font-size:14px;"><strong>Idioma:</strong> <a href="../pt_BR/field_service.md" style="text-decoration:none;font-weight:bold;">🇧🇷 Português</a> &nbsp;|&nbsp; <a href="../en_US/field_service.md" style="text-decoration:none;">🇺🇸 English</a></div>
## Field Service

> **48 recursos**

Aprenda sobre os recursos e atualizações mais recentes para aprimorar a produtividade e as operações da equipe dentro e fora do campo.

| Recurso | Usuários | Admins | Config | Contato | Docs |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Notas de versão mensal do Field Service** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Notas do patch do Field Service** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Notas de patch mensais do desktop** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Notas de patch mensais móveis** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Agentforce para Field Service** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Criar agentes de agendamento do Field Service no novo Agentforce Builder** | ❌ | ❌ | ✅ | ❌ | |
| **Expanda o agendamento autônomo para clientes potenciais** | ❌ | ❌ | ✅ | ❌ | |
| **Estender o alcance do agente de agendamento para email** | ❌ | ❌ | ✅ | ❌ | |
| **Estenda o alcance do agente de agendamento para alcance do cliente para WhatsApp** | ❌ | ❌ | ✅ | ❌ | |
| **Permitir que os trabalhadores gerenciem compromissos com Agentforce Employee Agent** | ❌ | ❌ | ✅ | ❌ | |
| **Personalizar porta-arquivos pré-trabalho com mais flexibilidade e velocidade** | ✅ | ❌ | ❌ | ❌ | |
| **Resolva problemas mais rapidamente com resumos interativos pré-trabalho** | ✅ | ❌ | ❌ | ❌ | |
| **Engajamento do cliente do Field Service** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Simplifique o suporte remoto roteando o Assistente remoto visual por meio do Omni-Channel** | ❌ | ❌ | ✅ | ❌ | |
| **Oferecer suporte a sessões seguras de vários aplicativos no Assistente remoto visual** | ❌ | ❌ | ✅ | ❌ | |
| **Gerenciar participantes da sessão para melhor colaboração e segurança no Assistente remoto visual (VRA)** | ❌ | ❌ | ✅ | ❌ | |
| **Fornecer suporte ininterrupto alternando modos perfeitamente em uma sessão do Assistente remoto visual (VRA)** | ✅ | ❌ | ❌ | ❌ | |
| **Field Service Mobile** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Captura de dados** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Aumente a produtividade com fluxos de captura de dados pré-preenchidos de uma fonte de dados usando repetidores** | ❌ | ✅ | ❌ | ❌ | |
| **Expanda atualizações e exclusões de registro e obtenha mais flexibilidade para atribuir elementos em loops** | ❌ | ✅ | ❌ | ❌ | |
| **Aprimore a usabilidade personalizando o estilo e o layout da tela em fluxos de captura de dados** | ❌ | ✅ | ❌ | ❌ | |
| **Mapas GIS nativos** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Melhore a precisão do local com edições de mapa e atualizações offline** | ❌ | ✅ | ❌ | ❌ | |
| **Operações do Field Service** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Impulsionar a eficiência operacional com Insights móveis (beta)** | ❌ | ❌ | ✅ | ❌ | |
| **Agendamento e otimização do Field Service** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Agendamento e otimização aprimorados** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Transforme sua experiência de agendamento com o novo console de agendamento** | ❌ | ❌ | ✅ | ❌ | |
| **Melhore a qualidade do roteamento com o objetivo de serviço do mesmo site aprimorado** | ❌ | ❌ | ✅ | ❌ | |
| **Aumentar a eficiência do trabalhador móvel agrupando compromissos de serviço nas proximidades** | ❌ | ❌ | ✅ | ❌ | |
| **Acelere os tempos de chegada ao serviço excluindo a viagem residencial no Agendamento e otimização aprimorados** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Melhore a precisão da agenda com estimativas de tempo de deslocamento personalizáveis para caminhões** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Obter recomendações de agendamento com Insights de compromisso (disponível ao público em geral)** | ❌ | ❌ | ✅ | ❌ | |
| **Melhore as recomendações de agendamento com a API de Insights de compromisso (disponível ao público em geral)** | ❌ | ✅ | ❌ | ❌ | |
| **Revise o histórico de solicitações de agendamento e otimização com relatórios de atividade (disponível ao público em geral)** | ❌ | ❌ | ✅ | ❌ | |
| **Mantenha compromissos importantes agendados durante a otimização (disponível ao público em geral)** | ❌ | ❌ | ✅ | ❌ | |
| **Corrija sobreposições de agendamento facilmente com um fluxo de agendamento automatizado (disponível ao público em geral)** | ❌ | ❌ | ✅ | ❌ | |
| **Identificar o Console de despacho clássico com seu nome de guia atualizado** | ❌ | ❌ | ✅ | ❌ | |
| **Simplificar a descoberta e a configuração de recursos do Field Service** | ❌ | ❌ | ✅ | ❌ | |
| **Agendamento de trabalho** ⚠️ | ❌ | ❌ | ❌ | ❌ | |

### Interações

| Recurso | Usuários | Admins | Config | Contato | Docs |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Agende quartos e equipamentos junto com a equipe com o agendamento de ativos para interações** | ❌ | ❌ | ✅ | ❌ | |
| **Permitir que os convidados gerenciem interações por email sem fazer login** | ❌ | ❌ | ✅ | ❌ | |
| **Ofereça o Gerenciamento de interação por autoatendimento em sites da experiência para clientes registrados** | ❌ | ❌ | ✅ | ❌ | |
| **Reservar recursos anonimamente** | ❌ | ❌ | ✅ | ❌ | |
| **Automatizar o gerenciamento de agendamento com turnos recorrentes** | ❌ | ❌ | ✅ | ❌ | |
| **Balancear cargas de trabalho entre recursos em serviço com o balanceamento de carga** | ❌ | ❌ | ✅ | ❌ | |
| **Mantenha compromissos agendados intactos ao converter leads** | ❌ | ❌ | ✅ | ❌ | |

## 🎓 Módulos Trailhead Relacionados

- [Field Service Lightning Basics](https://trailhead.salesforce.com/content/learn/modules/field-service-lightning-basics) — 1 hr 30 mins | 300 pts
- [Field Service Lightning Dispatch](https://trailhead.salesforce.com/content/learn/modules/field-service-dispatch) — 2 hrs | 400 pts

## 📚 Recursos

- [📄 Release in a Box PDF](./release-in-a-box.pdf)
- [🔗 Feature Impact Page](https://help.salesforce.com/s/articleView?id=release-notes.rn_feature_impact.htm&release=262&type=5&language=pt_BR)
- [📋 Release Notes](https://help.salesforce.com/s/articleView?id=release-notes.rn_release_notes.htm&release=262&type=5&language=pt_BR)
