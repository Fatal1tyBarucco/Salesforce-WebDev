<div style="padding:8px 12px;margin-bottom:16px;border:1px solid #d0d7de;border-radius:6px;background:#f6f8fa;font-family:system-ui,sans-serif;font-size:14px;"><strong>Idioma:</strong> <a href="../pt_BR/desenvolvimento.md" style="text-decoration:none;font-weight:bold;">🇧🇷 Português</a> &nbsp;|&nbsp; <a href="../en_US/desenvolvimento.md" style="text-decoration:none;">🇺🇸 English</a></div>
## Desenvolvimento

> **127 recursos**

Quer você esteja usando componentes do Lightning, o Visualforce, o Apex ou as APIs do Salesforce com sua linguagem de programação favorita, essas melhorias o ajudarão a desenvolver excelentes aplicativos, integrações e pacotes para revender a outras organizações.

| Recurso | Usuários | Admins | Config | Contato | Docs |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Componentes do Lightning** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Obtenha as alterações mais recentes do LWC com a API versão 67.0 do LWC** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Elementos de detalhes do grupo com o nome Atributo** | ❌ | ✅ | ❌ | ❌ | |
| **Melhorar o desempenho de recarregamento de módulo hot** | ❌ | ✅ | ❌ | ❌ | |
| **O desenvolvedor local agora é a visualização ativa** | ✅ | ❌ | ❌ | ❌ | |
| **Visualizar um único componente da Web Lightning no seu navegador (disponível ao público em geral)** | ✅ | ❌ | ❌ | ❌ | |
| **Visualizar um único componente da Web Lightning no Visual Studio Code (disponível ao público em geral)** | ✅ | ❌ | ❌ | ❌ | |
| **Simplificar interações de dados com gerentes de estado para LWC (disponível ao público em geral)** | ❌ | ✅ | ❌ | ❌ | |
| **Alterações de distorção da API no Lightning Web Security** | ❌ | ✅ | ❌ | ❌ | |
| **Esquema de URI de data: de blocos de distorção de HTMLAnchorElement** | ❌ | ✅ | ❌ | ❌ | |
| **Visualizar recursos com o Gerenciador de versão do Salesforce (beta)** | ❌ | ✅ | ❌ | ❌ | |
| **Carregar listas grandes dinamicamente (visualização do desenvolvedor)** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Obtenha um melhor suporte da direita para a esquerda** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Melhore o encapsulamento em componentes de base com alterações de método interno** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Suporte do Aura no Lightning Out 2.0 (disponível ao público em geral)** | ❌ | ❌ | ✅ | ❌ | |
| **Integre componentes do Lightning Out 2.0 por meio de sites do LWR do Experience Cloud** | ❌ | ❌ | ✅ | ❌ | |
| **Converta páginas de host Lightning Out (beta) para Lightning Out 2.0 usando a Ferramenta de migração MCP (piloto)** | ❌ | ❌ | ❌ | ✅ | |
| **Controle cookies SID particionados para o domínio do Lightning nas Configurações de sessão** | ❌ | ❌ | ✅ | ❌ | |
| **Integrar aplicativos da Web externos ao Salesforce com microfrontendas (prévia do desenvolvedor)** | ❌ | ❌ | ✅ | ❌ | |
| **Troque dados entre o Salesforce e widgets de MFE integrados (prévia do desenvolvedor)** | ❌ | ❌ | ✅ | ❌ | |
| **Aplicar o tema de marca a widgets de MFE integrados (prévia do desenvolvedor)** | ❌ | ❌ | ✅ | ❌ | |
| **Redimensione automaticamente widgets de MFE integrados para ajustar o conteúdo (visualização do desenvolvedor)** | ❌ | ❌ | ✅ | ❌ | |
| **Proteger alterações não salvas em widgets de MFE integrados (prévia do desenvolvedor)** | ❌ | ❌ | ✅ | ❌ | |
| **Navegar em widgets de MFE com leitores de tela e teclado (prévia do desenvolvedor)** | ❌ | ❌ | ✅ | ❌ | |
| **Atualizações da versão de componentes do Lightning** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Remover campos não públicos de dados de objeto personalizado em respostas de ação do Aura (atualização de versão)** | ❌ | ❌ | ✅ | ❌ | |
| **Tipos do Lightning** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Crie experiências móveis com tipos personalizados do Lightning** | ❌ | ✅ | ❌ | ❌ | |
| **Atribuir tipos personalizados do Lightning ao SDK móvel do Agentforce** | ❌ | ✅ | ❌ | ❌ | |
| **Visualizar componentes do Lightning Type usando a visualização em tempo real** | ❌ | ✅ | ❌ | ❌ | |
| **Gerar pacotes personalizados do tipo Lightning com habilidades do Agentforce Vibes** | ❌ | ✅ | ❌ | ❌ | |
| **Lightning Design System** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Atualizações de modelos de componentes do Sistema de design do Salesforce Lightning** | ✅ | ❌ | ❌ | ❌ | |
| **Obtenha orientação de design mais rápida com SLDS MCP Tools (beta)** | ✅ | ❌ | ❌ | ❌ | |
| **Usar o modo escuro em mais edições e recursos (beta)** | ❌ | ❌ | ✅ | ❌ | |
| **Operações de banco de dados são executadas no modo de usuário por padrão, não no modo do sistema** | ❌ | ✅ | ❌ | ❌ | |
| **Classes do Apex aplicam regras de compartilhamento por padrão** | ❌ | ✅ | ❌ | ❌ | |
| **A cláusula SOQL WITH SECURITY_ENFORCED foi removida** | ❌ | ✅ | ❌ | ❌ | |
| **Acionadores do Apex sempre são executados em um contexto "sem compartilhamento"** | ❌ | ✅ | ❌ | ❌ | |
| **Escrever testes de integração para Agentforce e Data 360 no Apex (Visualização do desenvolvedor)** | ❌ | ✅ | ❌ | ❌ | |
| **Evite interrupções de fluxo de trabalho habilitando limites elásticos para trabalhos assíncronos (beta)** | ❌ | ❌ | ✅ | ❌ | |
| **Bloquear a execução de código anônimo do Apex de pacotes gerenciados (atualização de versão)** | ❌ | ❌ | ✅ | ❌ | |
| **Escreva código mais limpo usando strings de várias linhas** | ❌ | ✅ | ❌ | ❌ | |
| **Trabalhar com Apex code no console da Web (beta)** | ❌ | ❌ | ✅ | ❌ | |
| **Obtenha as principais métricas do Apex em configuração com Agentforce (beta)** | ❌ | ❌ | ✅ | ❌ | |
| **Usar o Visualforce PDF Rendering Service com Apex Blob.toPdf() (atualização de versão)** | ❌ | ❌ | ✅ | ❌ | |
| **Personalizar a configuração de ação do Apex no Flow Builder** | ❌ | ✅ | ❌ | ❌ | |
| **Forneça construtores visíveis sem argumentos para classes personalizadas do Apex usadas como parâmetros de ação invocáveis** | ❌ | ❌ | ✅ | ❌ | |
| **Conecte agentes de IA ao Salesforce com segurança com servidores MCP hospedados (disponibilidade geral)** | ✅ | ❌ | ❌ | ❌ | |
| **Permitir que os assistentes de IA localizem e criem metadados do Salesforce com mais eficiência (beta)** | ❌ | ✅ | ❌ | ❌ | |
| **Criar ações do agente usando a API de consulta nomeada (disponível ao público em geral)** | ❌ | ✅ | ❌ | ❌ | |
| **As versões 31.0 a 40.0 da API da Salesforce Platform estão sendo descontinuadas** | ❌ | ✅ | ❌ | ❌ | |
| **Restringir o acesso de login à API SOAP** | ❌ | ✅ | ❌ | ❌ | |
| **Usar tokens de acesso baseados em JWT com a API SOAP** | ❌ | ✅ | ❌ | ❌ | |
| **Consultar o Data 360 com mais precisão** | ❌ | ✅ | ❌ | ❌ | |
| **Calcular valores em uma cláusula SOQL WHERE (piloto)** | ❌ | ✅ | ❌ | ❌ | |
| **Obtenha mais visibilidade do uso da API do seu agente com rastreamento aprimorado** | ❌ | ✅ | ❌ | ❌ | |
| **Atualizar URLs instanciados no tráfego de API (atualização de versão)** | ❌ | ❌ | ✅ | ❌ | |
| **Seja notificado sobre solicitações de API composta com tipos de evento EventLogFile** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Outros aprimoramentos na geração de um documento OpenAPI para a API REST sObjects (beta)** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Migrar seus eventos de plataforma de volume padrão para eventos de plataforma de alto volume** | ❌ | ✅ | ❌ | ❌ | |
| **Usar novos métodos EventBus.publishWithAccessLevel() para melhor controle de acesso** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Chamadas para ações invocáveis são validadas para requisitos de visibilidade do construtor na API versão 66.0 e posteriores** | ❌ | ❌ | ✅ | ❌ | |
| **A chamada de login() da API SOAP nas versões da API SOAP 31.0 a 64.0 está sendo descontinuada (atualização de versão)** | ❌ | ❌ | ✅ | ❌ | |
| **O login.database.com está sendo descontinuado** | ✅ | ❌ | ❌ | ❌ | |
| **Empacotamento** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Aplicar um período de expiração a upgrades por push personalizados** | ❌ | ✅ | ❌ | ❌ | |
| **Ambientes de desenvolvimento** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **The New Data Mask and Seed (disponível ao público em geral)** | ❌ | ✅ | ❌ | ❌ | |
| **Validar a prontidão de produção durante uma atualização de versão usando um sandbox que não seja de pré-visualização** | ❌ | ❌ | ❌ | ✅ | |
| **O limite de armazenamento da organização teste aumentou** | ❌ | ✅ | ❌ | ❌ | |
| **Ferramentas de desenvolvimento de plataforma** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Agentforce DX** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Acompanhe os aprimoramentos mais recentes do Agentforce DX** | ❌ | ✅ | ❌ | ❌ | |
| **Servidor Salesforce DX MCP** | ❌ | ✅ | ❌ | ❌ | |
| **Salesforce CLI** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Acompanhe os aprimoramentos mais recentes do Salesforce CLI** | ❌ | ✅ | ❌ | ❌ | |
| **Extensões do Salesforce para Visual Studio Code** | ❌ | ✅ | ❌ | ❌ | |
| **IDE do Agentforce Vibes** | ❌ | ✅ | ❌ | ❌ | |
| **Criar, depurar e implementar diretamente no Salesforce (beta)** | ❌ | ❌ | ✅ | ❌ | |
| **Extensão de Vibes do Agentforce** | ❌ | ✅ | ❌ | ❌ | |
| **Escalabilidade** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Planejar e teste de carga máxima com Teste de escala** | ❌ | ❌ | ❌ | ✅ | |
| **Implementar aplicativos dimensionáveis com o Centro de escala** | ❌ | ❌ | ❌ | ✅ | |
| **Otimize o código com o ApexGuru** | ❌ | ❌ | ❌ | ✅ | |
| **Aplicativos React** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Desenvolva aplicativos React com o Salesforce MultiFramework (disponível ao público em geral)** | ❌ | ❌ | ✅ | ❌ | |
| **Obtenha dados de registro em seus aplicativos React com o Data SDK e o GraphQL (beta)** | ❌ | ✅ | ❌ | ❌ | |
| **Salesforce Functions** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **O Salesforce Functions será descontinuado** | ❌ | ❌ | ❌ | ✅ | |
| **Parceiros do AgentExchange** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Proteger seu aplicativo conectado e soluções de aplicativo cliente externo** | ❌ | ✅ | ❌ | ❌ | |
| **Encontre soluções mais rapidamente com o novo AgentExchange** | ❌ | ✅ | ❌ | ❌ | |
| **Publicar soluções Agentforce com servidores MCP no AgentExchange** | ❌ | ✅ | ❌ | ❌ | |
| **Compartilhe sua excelência de segurança com um Partner Trust Center** | ❌ | ✅ | ❌ | ❌ | |
| **Captura de dados de alteração** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Receba notificações de evento de alteração para mais objetos** | ❌ | ✅ | ❌ | ❌ | |
| **Itens novos e alterados para desenvolvedores** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Componentes do Lightning** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Componentes da Web do Lightning novos e alterados** | ❌ | ✅ | ❌ | ❌ | |
| **Módulos novos e alterados para componentes da Web do Lightning** | ❌ | ✅ | ❌ | ❌ | |
| **Componentes Aura alterados** | ❌ | ✅ | ❌ | ❌ | |
| **Apex: Itens novos e alterados** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Namespace da ConnectApi** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Namespace de contexto** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Namespace de DocumentAI** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **namespace de hlthcrbilling** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Namespace invocável** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Namespace de CommercePayments** | ✅ | ❌ | ❌ | ❌ | |
| **namespace sfsqlquery** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Namespace do sistema** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **ConnectApi (Connect no Apex): Classes e enumerações novas e alteradas** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Alterações no limite de taxa do ConnectApi** | ✅ | ❌ | ❌ | ❌ | |
| **Novo Connect em classes do Apex** | ❌ | ✅ | ❌ | ❌ | |
| **Classes de saída do Connect no Apex alteradas** | ❌ | ✅ | ❌ | ❌ | |
| **Alteradas enumerações do Connect no Apex** | ❌ | ✅ | ❌ | ❌ | |
| **API: Itens novos e alterados** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Objetos novos e alterados** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Eventos de plataforma padrão novos e alterados** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Conectar a API REST** | ❌ | ✅ | ❌ | ❌ | |
| **API REST do CRM Analytics** | ✅ | ❌ | ❌ | ❌ | |
| **API de metadados** | ❌ | ✅ | ❌ | ❌ | |
| **API REST de relatórios e painéis** | ❌ | ✅ | ❌ | ❌ | |
| **API SOAP** | ❌ | ✅ | ❌ | ❌ | |
| **Objetos novos e alterados na API do conjunto de ferramentas** | ❌ | ✅ | ❌ | ❌ | |
| **API de Interface de usuário** | ❌ | ✅ | ❌ | ❌ | |
| **API do GraphQL** | ❌ | ✅ | ❌ | ❌ | |

## 🎓 Módulos Trailhead Relacionados

- [Apex Basics & Database](https://trailhead.salesforce.com/content/learn/modules/apex_basics_database) — 5 hrs | 1000 pts
- [Lightning Web Components Basics](https://trailhead.salesforce.com/content/learn/modules/lex_aura_lwc_lgpd) — 3 hrs | 600 pts
- [Apex Triggers](https://trailhead.salesforce.com/content/learn/modules/apex_triggers) — 1 hr 30 mins | 300 pts
- [Test Automation Strategies](https://trailhead.salesforce.com/content/learn/modules/test_automation_strategies) — 1 hr 30 mins | 300 pts

## 📚 Recursos

- [📄 Release in a Box PDF](./release-in-a-box.pdf)
- [🔗 Feature Impact Page](https://help.salesforce.com/s/articleView?id=release-notes.rn_feature_impact.htm&release=262&type=5&language=pt_BR)
- [📋 Release Notes](https://help.salesforce.com/s/articleView?id=release-notes.rn_release_notes.htm&release=262&type=5&language=pt_BR)
