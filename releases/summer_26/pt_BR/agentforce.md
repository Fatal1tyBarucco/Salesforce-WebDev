<div style="padding:8px 12px;margin-bottom:16px;border:1px solid #d0d7de;border-radius:6px;background:#f6f8fa;font-family:system-ui,sans-serif;font-size:14px;"><strong>Idioma:</strong> <a href="../pt_BR/agentforce.md" style="text-decoration:none;font-weight:bold;">🇧🇷 Português</a> &nbsp;|&nbsp; <a href="../en_US/agentforce.md" style="text-decoration:none;">🇺🇸 English</a></div>
## Agentforce

> **37 recursos**

Selecione Gemini para Agentforce	 	 	Yes

| Recurso | Usuários | Admins | Config | Contato | Docs |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Transcrição mais nítida e voz mais natural para Agentforce Voice** | ✅ | ❌ | ❌ | ❌ | |
| **Converse com Agentforce em mais 24 idiomas (beta)** | ❌ | ✅ | ❌ | ❌ | |
| **Obtenha percepções sobre o consumo de IA generativa e Agentforce** | ❌ | ✅ | ❌ | ❌ | |
| **Criar agentes no Criador novo apenas a partir de julho de 2026** | ❌ | ✅ | ❌ | ❌ | |
| **Editar subagentes na biblioteca de ativos** | ❌ | ✅ | ❌ | ❌ | |
| **Observabilidade do Agentforce: Análise de agente refinada e poncionadores personalizados (beta)** | ❌ | ✅ | ❌ | ❌ | |
| **Desbloqueie a interoperabilidade do agente com MCP para Agentforce** | ❌ | ✅ | ❌ | ❌ | |
| **Proteja conexões com suas APIs externas e servidores MCP com Políticas Agentforce** | ❌ | ❌ | ✅ | ❌ | |
| **Criar e implementar agentes habilitados para Voice no novo Agentforce Builder** | ❌ | ✅ | ❌ | ❌ | |
| **Extensão do suporte a idiomas globais com Agentforce Voice (beta)** | ✅ | ❌ | ❌ | ❌ | |
| **O provedor de pesquisa OpenAI na ação Pesquisar o agente da Web agora está disponível ao público em geral** | ❌ | ✅ | ❌ | ❌ | |
| **Aumente o Trust com citações em Traga seu próprio canal** | ❌ | ❌ | ✅ | ❌ | |
| **Atualize facilmente os agentes do Criador legado para o Criador novo** | ❌ | ✅ | ❌ | ❌ | |
| **Crie um agente avançado com script do agente para terminar com o Guia de implementação atualizado** | ❌ | ✅ | ❌ | ❌ | |
| **Encaminhe chamadas de voz do Agentforce usando SIP** | ❌ | ✅ | ❌ | ❌ | |
| **Transforme sua experiência do cliente com agentes habilitados para Voice no Chat v2 aprimorado** | ❌ | ✅ | ❌ | ❌ | |
| **Aprimore suas implantações do Chat v2 aprimoradas com mais personalização e confiabilidade** | ❌ | ✅ | ❌ | ❌ | |
| **Orquestrar outros agentes (beta)** | ❌ | ✅ | ❌ | ❌ | |
| **Prepare-se para o modelo atualizado para a opção hospedada pela AWS no Agentforce** | ❌ | ✅ | ❌ | ❌ | |
| **Funcionalidade de script do agente nova e alterada** | ❌ | ✅ | ❌ | ❌ | |
| **Ações e subagentes padrão do agente novos e alterados** | ❌ | ❌ | ✅ | ❌ | |
| **Desenvolvimento do Agentforce** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Gerenciar bibliotecas de dados do Agentforce com a API ADL Connect (beta)** | ❌ | ✅ | ❌ | ❌ | |
| **Criador de prompts** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Crie modelos inteligentes de prompts com lógica condicional (beta)** | ✅ | ❌ | ❌ | ❌ | |
| **Escreva avisos mais rapidamente com o Editor de bloco (beta)** | ✅ | ❌ | ❌ | ❌ | |
| **Usar atalhos de teclado no Criador de prompts** | ✅ | ❌ | ❌ | ❌ | |
| **Visualizar dependências de modelo de prompts em toda a sua organização** | ✅ | ❌ | ❌ | ❌ | |
| **Implemente e controle de versão modelos de prompt com a API de metadados** | ✅ | ❌ | ❌ | ❌ | |
| **Personalizar modelos de prompts gerenciados com substituições** | ✅ | ❌ | ❌ | ❌ | |
| **Governar idiomas de resposta de prompt** | ✅ | ❌ | ❌ | ❌ | |
| **Modelos com suporte** ⚠️ | ❌ | ❌ | ❌ | ❌ | |
| **Use o Gemini 3.5 Flash na Plataforma Einstein** | ❌ | ✅ | ❌ | ❌ | |
| **Limites maiores para solicitações de geração de modelo de idioma grande** | ❌ | ✅ | ❌ | ❌ | |
| **Usar o Nemotron 3 Super 120B na Plataforma Einstein (beta)** | ❌ | ❌ | ✅ | ❌ | |
| **Claude Opus 4.6 está disponível ao público em geral** | ❌ | ✅ | ❌ | ❌ | |
| **Preparar-se para a data de redirecionamento do Claude Sonnet 4** | ❌ | ✅ | ❌ | ❌ | |

## 🎓 Módulos Trailhead Relacionados

- [Agentforce Basics](https://trailhead.salesforce.com/content/learn/modules/agentforce-basics) — 1 hr 30 mins | 300 pts
- [Build an Agent with Agentforce](https://trailhead.salesforce.com/content/learn/projects/build-an-agent-with-agentforce) — 1 hr 30 mins | 500 pts
- [Agentforce for Developers](https://trailhead.salesforce.com/content/learn/modules/agentforce-for-developers) — 2 hrs | 400 pts

## 📚 Recursos

- [📄 Release in a Box PDF](./release-in-a-box.pdf)
- [🔗 Feature Impact Page](https://help.salesforce.com/s/articleView?id=release-notes.rn_feature_impact.htm&release=262&type=5&language=pt_BR)
- [📋 Release Notes](https://help.salesforce.com/s/articleView?id=release-notes.rn_release_notes.htm&release=262&type=5&language=pt_BR)
