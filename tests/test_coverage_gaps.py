...
# ── llm_service.py tests ────────────────────────────────────────


class TestLLMServiceCoverage:
    """Cover llm_service.py uncovered lines."""

    @pytest.mark.asyncio
    async def test_rate_limiter_acquire(self) -> None:
        from src.llm_service import RateLimiter
    ...
    @pytest.mark.asyncio
    async def test_rate_limiter_window_purge(self) -> None:
        from src.llm_service import RateLimiter
    ...
    def test_llm_provider_dataclass(self) -> None:
        from src.llm_service import LLMProvider

        p = LLMProvider(name="test", api_key="key123")
        assert p.name == "test"
    ...
    def test_circuit_breaker_config(self) -> None:
        from src.llm_service import CircuitBreakerConfig

        cfg = CircuitBreakerConfig(threshold=5, cooldown=30.0)
    ...
    def test_prompt_hash_deterministic(self) -> None:
        ...
        h1 = LLMService._prompt_hash("sys", "user")
        h2 = LLMService._prompt_hash("sys", "user")
        assert h1 == h2
        assert len(h1) == 64

    def test_prompt_hash_different(self) -> None:
        from src.llm_service import LLMService

        h1 = LLMService._prompt_hash("sys1", "user")
        h2 = LLMService._prompt_hash("sys2", "user")
        assert h1 != h2

    def test_load_providers_from_env_google(self) -> None:
        from src.llm_service import LLMService

        with patch.dict("os.environ", {"GOOGLE_API_KEY": "gkey"}, clear=False):
            svc = LLMService.__new__(LLMService)
            svc._logger = MagicMock()
            providers = svc._load_providers_from_env()
            names = [p.name for p in providers]
            assert "google" in names

    def test_load_providers_from_env_all(self) -> None:
        from src.llm_service import LLMService
    ...
    def test_select_providers_by_tier_cheap(self) -> None:
        ...
        providers = [
            ...
            LLMProvider(name="google", api_key="k", provider_type="google"),
            LLMProvider(name="deepseek", api_key="k"),
        ]
        svc = LLMService.__new__(LLMService)
        svc._providers = providers
        result = svc._select_providers_by_tier("cheap")
    ...
    def test_select_providers_by_tier_premium(self) -> None:
        ...
        svc = LLMService.__new__(LLMService)
        svc._providers = providers
        result = svc._select_providers_by_tier("premium")
        assert result[0].name == "google"
    ...
    def test_select_providers_by_tier_standard(self) -> None:
        ...
        svc = LLMService.__new__(LLMService)
        svc._providers = providers
        result = svc._select_providers_by_tier("standard")
        assert result == providers

    def test_select_providers_empty(self) -> None:
        from src.llm_service import LLMService
    ...
    def test_is_provider_available(self) -> None:
        ...
        svc = LLMService.__new__(LLMService)
        svc._provider_states = {"test": CircuitBreaker(threshold=3, cooldown=60)}
    ...
    def test_get_or_create_client_openai(self) -> None:
        ...
        svc = LLMService.__new__(LLMService)
        svc._clients = {}
        with patch("src.llm_service.openai.AsyncOpenAI") as mock_cls:
            client = svc._get_or_create_client(p)
            assert mock_cls.called
            client2 = svc._get_or_create_client(p)
    ...
    def test_get_or_create_client_google(self) -> None:
        ...
        svc = LLMService.__new__(LLMService)
        svc._clients = {}
        with patch("src.llm_service.genai.Client") as mock_cls:
            svc._get_or_create_client(p)
            assert mock_cls.called
    ...
    async def test_classify_text_invalid_json(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers)
        svc.generate_text = AsyncMock(return_value="not json")
        result = await svc.classify_text("test text", ["Security"])
    ...
    async def test_classify_text_none_response(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers)
        svc.generate_text = AsyncMock(return_value=None)
        result = await svc.classify_text("test text", ["Security"])
        assert "error" in result
    ...
    async def test_classify_text_custom_system_prompt(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers)
        svc.generate_text = AsyncMock(return_value='{"AI": {"applies": true, "confidence": 0.8}}')
        result = await svc.classify_text("test", ["AI"], system_prompt="Custom prompt")
        assert "AI" in result
    ...
    async def test_generate_batch_single_prompt(self) -> None:
        ...
        results = await svc.generate_batch(["prompt1"])
    ...
    async def test_generate_batch_multiple_prompts(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers)
        svc.generate_text = AsyncMock(return_value='["r1", "r2"]')
        results = await svc.generate_batch(["p1", "p2"])
    ...
    async def test_generate_batch_fallback_to_individual(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers)
        svc.generate_text = AsyncMock(side_effect=["bad json", "r1", "r2"])
        results = await svc.generate_batch(["p1", "p2"], batch_size=2)
        assert results[0] == "r1"
        assert results[1] == "r2"
    ...
    async def test_generate_batch_partial_json(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers)
        svc.generate_text = AsyncMock(side_effect=['["only_one"]', "r1", "r2"])
        results = await svc.generate_batch(["p1", "p2"], batch_size=2)
        assert results[0] == "r1"
    ...
    async def test_generate_text_with_cache_hit(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers, cache=cache)
    ...
    async def test_generate_text_with_cache_set(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers, cache=cache)
        svc._call_provider = AsyncMock(return_value="fresh response")
    ...
    async def test_generate_text_all_providers_fail(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers)
        svc._call_provider = AsyncMock(side_effect=Exception("fail"))
        svc._is_provider_available = MagicMock(return_value=True)
    ...
    async def test_generate_text_provider_unavailable(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers)
        svc._is_provider_available = MagicMock(return_value=False)
        result = await svc.generate_text("hello")
        assert result is None
    ...
    async def test_generate_text_timeout_error(self) -> None:
        ...
        providers = [LLMProvider(name="test", api_key="k")]
        svc = LLMService(providers=providers)
        svc._call_provider = AsyncMock(side_effect=TimeoutError("timeout"))
        svc._is_provider_available = MagicMock(return_value=True)
        svc._record_failure = MagicMock()
        result = await svc.generate_text("hello")
        assert result is None
    ...
    @pytest.mark.asyncio
    async def test_generate_text_legacy_client_fallback(self) -> None:
        from src.llm_service import LLMService

        svc = LLMService.__new__(LLMService)
        svc._providers = []
        svc._provider_states = {}
        svc._clients = {}
        svc._cache = None
    ...
    @pytest.mark.asyncio
    async def test_generate_text_legacy_client_error(self) -> None:
        from src.llm_service import LLMService
    ...
    async def test_call_google_provider(self) -> None:
        ...
        svc._clients["google"] = mock_client

        result = await svc._call_google_provider(p, "sys", "user")
    ...
    async def test_call_openai_provider_empty_choices(self) -> None:
        ...
        mock_client.chat.completions.create.return_value = mock_response
        svc._clients["test"] = mock_client

        result = await svc._call_openai_provider(p, "sys", "user")
        assert result == ""
    ...
    async def test_call_openai_provider_string_response(self) -> None:
        ...
        mock_client = AsyncMock()
        mock_client.chat.completions.create.return_value = "string response"
        svc._clients["test"] = mock_client
    ...
    async def test_aenter_aexit(self) -> None:
        ...
        svc = LLMService(providers=providers)
        async with svc:
            assert svc._clients is not None
