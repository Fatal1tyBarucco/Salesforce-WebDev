...
async def test_fc_validated_dict_path() -> None:
    from src.feature_classifier import FeatureClassifier, parse_classification_response
    mock_llm = AsyncMock()
    mock_llm.classify_text = AsyncMock(return_value={
        "ImpactLevel": {"value": "high"},
        "FeatureType": {"value": "security"},
        "justification": "j", "audience": "a", "priority": "high",
    })
...
def test_health_inc_metric_all_names() -> None:
    ...
    state = HealthState()
    for name in ["pipeline_runs_total", "pipeline_failures_total",
                 "features_processed_total", "scraper_requests_total",
                 "scraper_failures_total", "circuit_breaker_trips_total"]:
