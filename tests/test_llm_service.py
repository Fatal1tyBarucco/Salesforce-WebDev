...
async def test_generate_text_success(llm_service):
    ...
    with patch.object(
        llm_service, "generate_text", new_callable=AsyncMock
    ) as mock_gen:
...
async def test_summarize_release_notes(llm_service):
    ...
    with patch.object(
        llm_service, "summarize_release_notes", new_callable=AsyncMock
    ) as mock_sum:
...
async def test_enrich_feature(llm_service):
    ...
    with patch.object(
        llm_service, "enrich_feature", new_callable=AsyncMock
    ) as mock_enrich:
